...
def get_network(rdscr_df, plot_flag, pvalues, community_flag=1, verbose=0):
    ...
    for p_i in pvalues:
        ...
        p1p2 = pd.concat([p1, p2], ignore_index=True)
    ...
    # num_nodes = len(pd.unique(stable_interactions[['v1','v2']].values.ravel('K')))
    
    # tmp_stable_interactions = stable_interactions.copy()
    # tmp_stable_interactions['count_decile'] = pd.qcut(tmp_stable_interactions['count'], 
    #                                                   10, 
    #                                                   labels=False, 
    #                                                   duplicates='drop')
    # # tmp_num_nodes = num_nodes
    # for deciles in range(0,10):
    #     if tmp_num_nodes == num_nodes:
    #         tmpg = tmp_stable_interactions[tmp_stable_interactions['count_decile'] > deciles]
    #         tmp_num_nodes = len(pd.unique(tmpg[['v1','v2']].values.ravel('K')))
    
        
    #cutoff = np.linspace(0.9,0.1,12)
    # tmp_num_nodes = num_nodes
    # while tmp_num_nodes == num_nodes:
    #     min_idx = tmp_stable_interactions['count'].idxmin()
    #     tmp_stable_interactions.drop(min_idx, inplace=True)
    #     tmp_num_nodes = len(pd.unique(tmp_stable_interactions[['v1','v2']].values.ravel('K')))

    #print("Removed " + str(stable_interactions.shape[0] - tmpg.shape[0]) + " edges")
    #graph_interactions = tmpg.copy()
   
    graph_interactions = stable_interactions.copy()
...
def rank_nodes(G, nodes, verbose=0):
    ...
    eigenvector_dict = nx.eigenvector_centrality(G, weight='count', max_iter=1000) 
    # voterank 
